...
const techs_json = fs.readFileSync(path.resolve(__dirname, 'techs.json'), 'utf8'); // tech names in techs.json must match tech names in wappalyzer/apps.json
const techs = JSON.parse(techs_json);

/**
 * Tech class
 * scanPlugin, plugin_slug, coreVersion should only be used when scanning a plugin
 */
const Tech = function (techname, scanPlugin, plugin_slug, coreVersion) {
  ...
  */
  ...
  findPlugins: function (techVersion, progressCB, doneCB) {
    ...
    try {
      ...
      var fn = function (pluginsPath, plugin_slug, callback) {
                  ...
                  if (response.statusCode === 200) {
                    ...
                    if (match && match.length > 1) {
                      ...
                      if (fs.existsSync(jsonFilePath)) { // file must exist
                        ...
                        let pluginFormatted = _this.formatPluginOutput(app_data)
                        detected_plugins_data.push(pluginFormatted)
                      } else {
  ...
  formatPluginOutput : function(pIn) {
    ...
    try {
      ...
      if (pIn.latest_version) {
        let latest_version = new Version(pIn.latest_version)
    ...
    if (pIn.vulnerabilities) {
      ...
      for (let vuln of pIn.vulnerabilities) {
        try {
          let vuln_fixed_in = new Version(vuln.fixed_in)
