...
describe('Class Tech', function () {
    ...
    request(requestOptions, (err, response, body) => {
      if (err) {
        ...
        if (response.statusCode == 200 || response.statusCode == 206) {
          tech.findRoots(response.request.uri.href, // response.request.uri contains the response uri, potentially redirected
            body);
          tech.isVersionOrNewer(new Version("2.5.1"), (err, result, proofs) => {
            if (err) {
              done(err);
            } else {
    ...
    request(Tech.getReqOptions("http://wordpress3-8.whatsbehind.io/"), function (err, response, body) {
      if (err) done(err);
        ...
        if (response.statusCode == 200 || response.statusCode == 206) {
          tech.findRoots(response.request.uri.href, // response.request.uri contains the response uri, potentially redirected
            body);
          tech.isVersionOrNewer(new Version("3.8"), function (err, result, proofs) {
            if (err) done(err);
            else {
              assert.ok(result == "success");
              done();
  ...
  it('crlf2lf', function () {
    const data_crlf = fs.readFileSync("./test/data/github_crlf.html");
    const data_lf = fs.readFileSync("./test/data/github_lf.html");
    const converted_trim = Tech.crlf2lf(data_crlf);
    
    assert.equal(converted_trim.length, data_lf.length);
    var same = true;
