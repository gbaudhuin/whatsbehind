...
describe('Class Tech', function () {
    ...
    it('isVersionOrNewer 2.5.1', function (done) {
        ...
        request(Tech.getReqOptions("http://wordpress2-5-1.whatsbehind.io/"), function (err, response, body) {
            if (err) done(err);
            else {
                if (response.statusCode == 200 || response.statusCode == 206) {
                    tech.findRoots(response.request.uri.href, // response.request.uri contains the response uri, potentially redirected
                        body);
                    tech.isVersionOrNewer(new Version("2.5.1"), function (err, result, proofs) {
                        if (err) done(err);
                        else {
